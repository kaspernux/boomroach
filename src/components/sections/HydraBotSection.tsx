...
function LiveTradingTerminal() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const newLines = [
        `[${new Date().toLocaleTimeString()}] Monitoring market conditions...`,
        `[${new Date().toLocaleTimeString()}] ML model confidence: ${(85 + Math.random() * 15).toFixed(1)}%`,
        `[${new Date().toLocaleTimeString()}] Portfolio balance: $${(47239 + Math.random() * 1000).toFixed(2)}`,
        `[${new Date().toLocaleTimeString()}] Active strategies: ${strategies.filter(s => s.status === 'active').length}/4`
      ]
