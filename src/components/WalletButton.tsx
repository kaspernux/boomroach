...
export function WalletButton() {
    ...
    if (connecting) {
      ...
      const timeout = setTimeout(() => {
        if (connecting && !connected) {
          setConnectionError("Connection taking longer than expected. Please try again.");
        }
  ...
  const retryConnection = async () => {
    ...
    try {
      ...
      setConnectionError("Failed to retry connection. Please refresh the page and try again.");
      setIsRetrying(false);
    }
  };

  if (connecting || isRetrying) {
    return (
      <motion.div
      ...
      >
        ...
        {connectionError && (
          <motion.div
            ...
            className="mt-2 p-2 bg-red-500/10 border border-red-500/30 rounded text-xs text-red-400"
          >
            <div className="flex items-center space-x-2">
            ...
            <Button
              ...
              className="mt-2 bg-red-500/20 text-red-400 hover:bg-red-500/30 text-xs"
            >
              Retry Connection
            </Button>
          </motion.div>
